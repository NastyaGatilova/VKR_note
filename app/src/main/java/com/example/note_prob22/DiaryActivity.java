...
//            record.setId(i + 1); // Перенумеровать ID, начиная с 1
//        }
...
//        }
        ...
        Log.d("--Help--", "Длина списка = "+ Record.noteDayArrayList.size());
//        for (int i = 0; i < Record.noteDayArrayList.size(); i++) {
//            Record record = Record.noteDayArrayList.get(i);
//            record.setId(i + 1); // Перенумеровать ID, начиная с 1
//        }
       // diaryAdapter.notifyDataSetChanged();
        for (Record item : Record.noteDayArrayList) {
            Log.d("--Help--", "Запись id = " + item.getId() + " чувство=" + item.getFeeling() );
